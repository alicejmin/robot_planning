...
#Todo: if path is a subset of existing path, dont change colors 
...
def a_star(graph, start_position, goal_position):
    ...
    while len(open_list) != 0:
        ...
        for child in children:
            if child in closed_list or graph.nodes[child]['position'] == (0, 0, 0) or graph.nodes[child]['position'][2] == 0:
                continue
...
def main():
    ...
    path_pub = rospy.Publisher('/planned_path', String, queue_size=10, tcp_nodelay=False)
    
    # define the listener
    rospy.Subscriber('/robot_pos', Point, rob_pos_callback, queue_size=10)

    rospy.Subscriber('/goal_pos', Point, goal_pos_callback, queue_size=10)

    mission_hz = 1
    r = rospy.Rate(mission_hz)
    ...
    while not rospy.is_shutdown():
        with start_mutex, goal_mutex:
            if start is None or goal is None or np.any(np.isnan(start)) or np.any(np.isnan(goal)): continue
